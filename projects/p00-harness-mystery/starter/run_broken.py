#!/usr/bin/env python3
"""Run the broken-harness arm of the harness mystery.

Creates an Agent Canvas conversation with a deliberately degraded harness config
and sends it the discover-prove-fix task. Capture the event stream to
events-broken.json and inspect it with diagnose.md.

This is the starter script. Do not read solution/run_repaired.py until you have
diagnosed the broken run.
...
def main():
    ...
    # The broken harness config — the three degraded settings.
    broken_cfg = {
        "native_tool_calling": False,   # force tool calls as text, not parsed
        "reasoning_effort": "none",     # drop the model's reasoning between turns
        "temperature": None,            # don't forward sampling params
        "top_p": None,
        "max_completion_tokens": None,
        "drop_params": True,            # let the provider drop unknown params
    }

    payload = build_payload(settings, workdir, broken_cfg)
    ...
    print(f"  curl -sS -H 'X-Session-API-Key: $_KEY' \\")
    print(f"    'http://localhost:18000/api/conversations/{cid}/events/search?limit=20' \\")
    print(f"    > events-broken.json")
    print(f"\nThen open diagnose.md and answer the questions from the trace.")
