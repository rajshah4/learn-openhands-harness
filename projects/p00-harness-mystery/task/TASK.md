...
2. **Prove** it. Write a file `poc.py` in the task directory that, when run with
...
3. **Fix** it. Write a file `patch.py` in the task directory that, when run with
...
4. **Verify** by running `python ../checker.py` from the task directory. The
...
- Work only inside this task directory. Do not modify anything outside it.
...
All four S0–S3 criteria pass. If you cannot complete a step, say so explicitly
